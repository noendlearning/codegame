	...
	return elm$http$Http$post(
		{
			body: elm$http$Http$multipartBody(
				_List_fromArray(
					[
						A2(elm$http$Http$stringPart, '', login.email),
						A2(elm$http$Http$stringPart, '', login.password)
					])),
			expect: elm$http$Http$expectString(author$project$Main$GotText),
			url: '/loginUser'
		});
	...
	return elm$http$Http$post(
		{
			body: elm$http$Http$multipartBody(
				_List_fromArray(
					[
						A2(elm$http$Http$stringPart, '', singup.email),
						A2(elm$http$Http$stringPart, '', singup.password)
					])),
			expect: elm$http$Http$expectString(author$project$Main$GotText),
			url: '/registerUser'
		});
...
var author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'GotText':
				return _Utils_Tuple2(model, author$project$Main$gohome);
			case 'NoNeeResult':
